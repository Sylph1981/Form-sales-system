...
#2021.12.06機能追加（お問い合わせフォーム投稿前に営業リストをプレビュー表示）
	        ...
	        self.m_staticText612 = wx.StaticText( sbSizer75.GetStaticBox(), wx.ID_ANY, u"sheet title", wx.DefaultPosition, wx.DefaultSize, 0 )
...
#現在のワークシートのタイトルをリストへ格納
	        ...
	        self.m_comboBox12 = wx.ComboBox( sbSizer75.GetStaticBox(), wx.ID_ANY, u"選択して下さい", wx.DefaultPosition, wx.DefaultSize, m_comboBox12Choices, wx.CB_DROPDOWN )
	        gSizer8.Add( self.m_comboBox12, 0, wx.ALL, 5 )

	        self.btn32 = wx.Button( sbSizer75.GetStaticBox(), wx.ID_ANY, u"Confirm", wx.DefaultPosition, wx.DefaultSize, 0 )
	        ...
	        sbSizer75.Add( gSizer8, 1, wx.EXPAND, 5 )


	        self.m_panel85.SetSizer( sbSizer75 )
	        self.m_panel85.Layout()
	        sbSizer75.Fit( self.m_panel85 )
	        bSizer15.Add( self.m_panel85, 1, wx.EXPAND |wx.ALL, 5 )
...
#	        gSizer8.Add( self.m_panel27, 1, wx.EXPAND |wx.ALL, 5 )
	        ...
	        self.row11 = wx.SpinCtrl( self.m_panel51, wx.ID_ANY, wx.EmptyString, wx.DefaultPosition, wx.Size( 100,-1 ), wx.SP_ARROW_KEYS, 0, 10000, 0 )
...
#	      self.row11.Disable()
	        ...
	        self.row21 = wx.SpinCtrl( self.m_panel51, wx.ID_ANY, wx.EmptyString, wx.DefaultPosition, wx.Size( 100,-1 ), wx.SP_ARROW_KEYS, 0, 10000, 0 )
...
#	        bSizer15.Add( gSizer8, 1, wx.EXPAND, 5 )
	        bSizer15.Add( self.m_panel271, 1, wx.EXPAND |wx.ALL, 5 )
...
#送信ボタン（aタグの指定要素のみ取得）
 		        element8 = soup.find_all("a",class_="btn_check")
 		        print(element8)
 		        list_8bc = []

 		        for elem in element8:
 		          list_8bc.append(elem.get("class"))

 		        list_8bc = [i for i in list_8bc if i is not None]
 		        print(list_8bc)
                    
#送信ボタン（imgタグの指定要素のみ取得）
 		        element10 = soup.find_all("img",alt="送信")
 		        print(element10)
 		        list_10ia = []

 		        for elem in element10:
 		          list_10ia.append(elem.get("alt"))
 		        list_10ia = [i for i in list_10ia if i is not None]
 		        print(list_10ia)
...
#id，value，name要素が存在する
 		           		              ...
 		           		              try:
 		           		                ...
 		           		                try:
 		           		                  ...
 		           		                  checkbox = driver.find_element_by_css_selector("[value='お問い合わせ']")
 		           		                  if checkbox.is_selected() is not True:
 		           		                    driver.execute_script("arguments[0].click();", checkbox)
 		           		                    print(checkbox.is_selected())
 		           		                    if checkbox.is_selected() is True:
...
#エラー回避
 		           		                ...
 		           		                try:
 		           		           		             checkbox = driver.find_elements_by_css_selector("[value='お問い合わせ']")[0]
 		           		           		             if checkbox.is_selected() is not True:
 		           		           		               driver.execute_script("arguments[0].click();", checkbox)
 		           		           		               print(checkbox.is_selected())
 		           		           		               if checkbox.is_selected() is True:
 		           		           		           		          lastcol = len(list(ws3.row_values(k)))    
 		           		           		           		          time.sleep(1) 		           		             
 		           		           		           		          ws3.update_cell(k, lastcol+1, "お問い合わせ")
...
#（name値に指定キーワードが含まれない、且つtype値に指定キーワードが含まれる）
                       ...
                       or len([i for i in list_2t if "submit" in i \
                               or "button" in i \
                                   or "image" in i]) > 0 \
                           or not len([i for i in list_10ia if "送信" in i]) == 0:
...
#パターン追加（2021.12.28）
#<li class="msg">前画面に戻って正しく入力してください。</li>
 		           		                                 for Content in soup.find_all("li",style=re.compile("msg")):
 		           		                                  Content = re.sub("[\n]", "", Content.getText(), 4)
 		           		                                  ws3.update_cell(k, 7, Content)
 		           		                                  format_cell_range(ws3, f'A{k}:Z{k}', data_fmt4)
 		           		                                 print(soup.find_all("li",style=re.compile("msg")))
...
# 		           		                                  list_12io = [i for i in list_12io if "sending" in i \
#                                                               and i is not None]
...
#（class値に指定キーワードが含まれる）
 		        elif "フォーム要素あり" in ws3.cell(k, 7).value \
                     and not len([i for i in list_2c if "btn" in i \
                                  or "submit" in i]) == 0 \
                         or not len(list_11dc) == 0:
...
#表示されたアラートポップアップ（アラートモーダル）をSeleniumで操作できるようにするため、alertオブジェクトを取得。
# 		           		          alert = driver.switch_to.alert
# 		           		          ws3.update_cell(k, 8, alert.text)
# 		           		          print(alert.text)
# 		           		          format_cell_range(ws3, f'A{k}:Z{k}', data_fmt4)
...
# 		           		           		           else:
 		               ...
 		               try:
 		           		        if "フォーム要素あり" in ws3.cell(k, 7).value:
 		           		              elem = driver.find_element_by_css_selector('.wpcf7-form-control.wpcf7-submit.btn.btn-primary')
 		           		              driver.execute_script("arguments[0].click();", elem)
 		           		              time.sleep(3)
 		           		              html = driver.page_source
 		           		              soup = BeautifulSoup(html, 'html.parser')
...
#結果出力
 		           		              for result in soup.select(".wpcf7-response-output"):
 		           		           		           ...
 		           		           		           if Content == "":
 		           		           		             ...
 		           		           		             ws3.update_cell(k, 8, Content)
 		           		           		             if "入力内容に" in ws3.cell(k, 7).value:
 		           		           		               format_cell_range(ws3, f'A{k}:Z{k}', data_fmt4)
 		           		           		             else:
 		           		           		               format_cell_range(ws3, f'A{k}:Z{k}', data_fmt5)
 		           		              print(soup.select(".wpcf7-response-output"))
...
#2021.12.09パターン追加（javascript対応）                                
 		           		          if "フォーム要素あり" in ws3.cell(k, 7).value:
 		           		            try:
#<a href="javascript:chk_privacy();">入力内容を確認する</a>
 		           		              list_11dc = []
 		           		              element11 = soup.find_all("a",href=re.compile("chk"))
 		           		              print(element11)
 		           		              for href in element11: 		           		                
 		           		                list_11dc.append(href.getText())
 		           		              list_11dc = [i for i in list_11dc if i is not None]
 		           		              print(list_11dc)                    
 		           		              driver.find_element_by_link_text(list_11dc[0]).click()
...
#エラーメッセージ
#<span class="txt_alert">こちらは必須項目です。</span>
 		           		              for result in soup.find_all(class_=re.compile("alert")):
 		           		                Content = re.sub("[\n]", "", result.getText(), 4)
 		           		                ws3.update_cell(k, 8, Content)
 		           		              format_cell_range(ws3, f'A{k}:Z{k}', data_fmt4)                                           
 		           		              print(soup.find_all(class_=re.compile("alert")))
...
#送信ボタン押下
 		           		          if "フォーム要素あり" in ws3.cell(k, 7).value:
 		           		              list_11dc = []
 		           		              element11 = soup.find_all("a",href=re.compile("submit"))
 		           		              print(element11)
 		           		              for href in element11: 		           		                
 		           		                list_11dc.append(href.getText())
 		           		              list_11dc = [i for i in list_11dc if i is not None]
 		                     ...
 		                     try:
 		           		          if "フォーム要素あり" in ws3.cell(k, 7).value:
 		           		              elem = driver.find_element_by_css_selector('.wpcf7-form-control.wpcf7-submit.p-contactform_submit_btn')
 		           		              driver.execute_script("arguments[0].click();", elem)
 		           		              time.sleep(3)
 		           		              html = driver.page_source
 		           		              soup = BeautifulSoup(html, 'html.parser')
...
#エラーメッセージ
 		                       ...
 		                       try:
 		           		            if "フォーム要素あり" in ws3.cell(k, 7).value:
 		           		              elem = driver.find_element_by_css_selector('.wpcf7-form-control.wpcf7-confirm.btn.wpcf7c-elm-step1.wpcf7c-btn-confirm')
 		           		              driver.execute_script("arguments[0].click();", elem)
 		           		              time.sleep(3)
 		           		              html = driver.page_source
 		           		              soup = BeautifulSoup(html, 'html.parser')
...
#エラーメッセージ
 		                         ...
 		                         try:
 		                           if "フォーム要素あり" in ws3.cell(k, 7).value:
 		                              elem = driver.find_element_by_css_selector('.wpcf7-form-control.has-spinner.wpcf7-submit.btn.btn-success.btn-lg')
 		                              driver.execute_script("arguments[0].click();", elem)
 		                              time.sleep(3)
 		                              html = driver.page_source
 		                              soup = BeautifulSoup(html, 'html.parser')
...
#送信ボタン押下
 		                               element13 = soup.find_all("input", type=re.compile("submit"))
 		                               print(element13)
 		                               list_13in = []

 		                               for elem in element13:
 		                                 list_13in.append(elem.get("name"))
 		                               list_13in = [i for i in list_13in if "Send" in i \
                                               and i is not None]
# 		                               print(list_13in)

 		                               if len(list_13in) > 0 \
                                            and "フォーム要素あり" in ws3.cell(k, 7).value:
 		                                 driver.find_element_by_name(list_13in[0]).click()
 		                                 time.sleep(3)
 		                                 if "エラー" in driver.title \
                                              or "SPAM BLOCK" in driver.title \
                                                  or "404 Not Found" in driver.title:
...
#（spanタグのclass値に指定キーワードが含まれる）
                     ...
                     and not len([i for i in list_12sc if "appsMaterialWizButtonPaperbuttonLabel" in i]) == 0:
...
#<span class="freebirdFormviewerComponentsQuestionBaseErrorText">この質問は必須です</span>
 		           		              for result in soup.find_all(class_=re.compile("Err")):
 		           		                Content = re.sub("[\n]", "", result.getText(), 4)
 		           		                ws3.update_cell(k, 7, Content)
 		           		              format_cell_range(ws3, f'A{k}:Z{k}', data_fmt4)                                           
 		           		              print(soup.find_all(class_=re.compile("Err")))
...
#パターン追加（2021.12.03）
#（pタグのclass値に指定キーワードが含まれる）
 		        elif "フォーム要素あり" in ws3.cell(k, 7).value \
                     and not len([i for i in list_8bc if "btn_check" in i]) == 0:
 		             print([i for i in list_8bc if "btn_check" in i])
                       
 		             try:
#送信ボタン押下
#<p id="btn_box"><a href="#" onclick="frmcheck();" class="btn_check">確認画面へ</a></p>
 		           		          if "フォーム要素あり" in ws3.cell(k, 7).value:
 		           		            driver.find_element_by_class_name("btn_check").click()
 		           		            time.sleep(3)
 		           		            html = driver.page_source
 		           		            soup = BeautifulSoup(html, 'html.parser')

#エラーメッセージ
 		           		          for result in soup.select("#attention"):
 		           		           	Content = re.sub("[\n]", "", result.getText(), 4)
 		           		           	ws3.update_cell(k, 7, Content)
 		           		           	format_cell_range(ws3, f'A{k}:Z{k}', data_fmt4)                                           
...
#送信ボタン押下
 		           		          if "フォーム要素あり" in ws3.cell(k, 7).value:
 		           		            elem = driver.find_element_by_class_name("btn_send")
 		           		            driver.execute_script("arguments[0].click();", elem)
 		           		            ws3.update_cell(k, 7, "送信内容確認後に送信完了しました。")
 		           		            format_cell_range(ws3, f'A{k}:Z{k}', data_fmt5)
...
#（確認ボタンの場合）
                     ...
                     and not len([i for i in list_2 if "Confirm" in i]) == 0 \
...
# 		           		            format_cell_range(ws3, f'A{k}:Z{k}', data_fmt5)
                     ...
                     and not len([i for i in list_2v if "確認" in i \
                         or "confirm" in i]) == 0 \
                       and not list_2v == []:
 		             print(list_2v)
 		             try:
...
#送信ボタン押下
 		           		          if "フォーム要素あり" in ws3.cell(k, 7).value:
 		           		            ...
 		           		            print(cell_list2)
 		           		            if not "メールアドレス" in cell_list2:
 		           		              ws3.update_cell(k, 7, "メールアドレスが入力されていません")
 		           		              format_cell_range(ws3, f'A{k}:Z{k}', data_fmt4)
 		           		            else:
 		           		              elem = driver.find_element_by_css_selector("input[type='submit']")
 		           		              driver.execute_script("arguments[0].click();", elem)
...
# 		           		                 pass
                     ...
                     and not len([i for i in list_2 if "submit" in i]) == 0:
 		             print([i for i in list_2 if "submit" in i])
 		             try:
...
#パターン追加（2021.12.05）
#画像タグを元にクリック（imgタグのalt値に指定キーワードが含まれる）
# 		        elif not len([i for i in list_10ia if "送信" in i]) == 0:

# 		           		                          try:
# 		           		                            if "フォーム要素あり" in ws3.cell(k, 7).value:
# 		           		                              elem = driver.find_element_by_css_selector("img[alt='送信']")
# 		           		                              driver.execute_script("arguments[0].click();", elem)
# 		           		                              time.sleep(3)
# 		           		                              ws3.update_cell(k, 7, driver.title)
# 		           		                              print(driver.title)
...
#法人名称～ホームページＵＲＬまで表示
		  ...
		  cell_list9 = ws3.range(1, 9, row4, 9)
...
#		    adid.grid.SetCellValue(row, 0, cell_list1[i].value)
		    ...
		    adid.grid.SetCellValue(row, 6, cell_list7[i].value)
		    adid.grid.SetCellValue(row, 7, cell_list8[i].value)
		    adid.grid.SetCellValue(row, 8, cell_list9[i].value)
...
#		self.grid.SetColLabelValue(7, "投稿日時")
		...
		self.grid.SetColLabelValue(4, "お問い合わせページ")
		self.grid.SetColLabelValue(5, "結果・エラー情報等（１）")
		self.grid.SetColLabelValue(6, "結果・エラー情報等（２）")
		self.grid.SetColLabelValue(7, "投稿日時")
